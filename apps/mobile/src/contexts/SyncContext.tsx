...
export const SyncProvider: React.FC<SyncProviderProps> = ({ children }) => {
  ...
  const [offlineTimeRemaining, setOfflineTimeRemaining] = useState<
    number | null
  >(null);
  const [cacheStats, setCacheStats] = useState<any>(null);

  // Multi-device state
  const [currentSession, setCurrentSession] = useState<DeviceSession | null>(
    null
  );
  ...
  const canOperateOffline =
    offlineTimeRemaining === null || offlineTimeRemaining > 0;
  ...
  useEffect(() => {
    ...
    const initializeSync = async () => {
      try {
        ...
        const offlineLimitListener = offlineDataService.subscribe(
        ...
        const cacheListener = offlineDataService.subscribe(
          'cacheCleared',
          () => {
  ...
  const enableOfflineMode = useCallback(async () => {
    try {
      ...
      console.log(
        'Offline mode is automatically enabled when connectivity is lost'
      );
    } catch (error) {
  ...
  const resolveConflict = useCallback(
    async (
      ...
      try {
        console.log(
          `Resolving conflict ${conflictId} with strategy: ${resolution}`
        );
  ...
  const joinWithAccessCode = useCallback(
    async (code: string): Promise<DeviceSession> => {
  ...
  const generateAccessCode = useCallback(
    async (
      ...
      console.log(
        `Generating access code for tournament ${tournamentId}, role: ${role}`
      );
      // Placeholder implementation
  ...
  const delegatePermissions = useCallback(
    async (
      deviceId: string,
      tournamentId: string,
      permissions: string[],
      ...
      console.log(
        `Delegating permissions to device ${deviceId}: ${permissions.join(', ')}`
      );
      // Placeholder implementation
    },
    []
  ...
  const hasPermission = useCallback(
    async (permission: string, matchId?: string): Promise<boolean> => {
  ...
  const onOfflineWarning = useCallback(
    (callback: (timeRemaining: number) => void) => {
