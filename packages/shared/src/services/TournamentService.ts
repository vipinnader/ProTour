...
export class TournamentService extends DatabaseService {
  ...
  async joinTournament(accessData: Omit<TournamentAccess, 'joinedAt'>): Promise<TournamentAccess> {
    ...
    const existingAccess = await this.query<TournamentAccess>(this.ACCESS_COLLECTION, [
      { fieldPath: 'tournamentId', opStr: '==', value: accessData.tournamentId },
    ...
    if (accessData.role === 'player' && tournament.currentPlayerCount >= tournament.maxPlayers) {
      throw new Error('Tournament is full');
    }
  ...
  async getPlayerRegistrations(userId: string): Promise<TournamentRegistration[]> {
  ...
  async getPlayerSchedule(userId: string, tournamentId: string): Promise<PlayerSchedule> {
    ...
    // Categorize matches
    const currentMatch = playerMatches.find(match => match.status === 'in-progress');
    ...
    const tournamentProgress = this.calculateBracketPosition(userId, playerMatches);

    // Estimate next match time (simple estimation)
    const estimatedNextMatchTime = upcomingMatches.length > 0 && upcomingMatches[0].startTime
  ...
  private calculateBracketPosition(userId: string, matches: Match[]): BracketPosition {
    const completedMatches = matches.filter(match => match.status === 'completed');
    const currentRound = Math.max(...matches.map(match => match.round), 0);
    
    // Check if player is eliminated
    const eliminated = completedMatches.some(match => 
  ...
  async searchPublicTournaments(filters: {
    sport?: Tournament['sport'];
    location?: string;
    status?: Tournament['status'];
    limit?: number;
    ...
    const tournaments = await this.query<Tournament>(this.COLLECTION, queryConstraints);
