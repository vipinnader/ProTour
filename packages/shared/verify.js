      ...
      try {
        ...
        const basicChecks = [
          {
            ...
            message: 'Missing semicolon after import',
          },
          {
            regex: /export.*{[^}]*}(?![;,])/g,
          ...
          },
          {
            ...
            message: 'Missing semicolon after interface',
          },
          {
            regex: /class\s+\w+[^{]*{[^}]*}(?![;,])/g,
            message: 'Missing semicolon after class (if needed)',
          },
        ];
        ...
        for (const line of lines) {
          if (
            ...
            issues.push(
              `${filePath}:${lineNumber} - Possible malformed import`
            );
          }
        ...
        if (totalOpenBraces !== totalCloseBraces) {
          issues.push(
            `${filePath} - Unmatched braces (${totalOpenBraces} open, ${totalCloseBraces} close)`
          );
        }
      } catch (error) {
...
if (issues.length === 0) {
  ...
  console.log(
    '💡 Install dependencies and run "npm run build" for complete validation.'
  );
} else {
